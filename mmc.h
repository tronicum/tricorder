...
#define CS_LOW()    P5OUT &= ~(1<<4);             // Card Select
#define CS_HIGH()   SPI_TX_DONE; P5OUT |= (1<<4)  // Card Deselect
...
#define MMC_BUSY              0x13
#define MMC_BUS_ERROR         0x14
...
char mmcWriteBlock (uint16_t header, const unsigned long address, unsigned long count, unsigned char *pBuffer);
#define mmcWriteSector(sector, pBuffer) mmcWriteBlock(0x0000, sector*512ul, 512, pBuffer)
#define my_mmcWriteSector(header, sector, pBuffer) mmcWriteBlock(header, sector*512ul, 510, pBuffer)
#define mmcWriteBlankSector(sector) mmcWriteBlock(sector*512ul, 512, 0)
